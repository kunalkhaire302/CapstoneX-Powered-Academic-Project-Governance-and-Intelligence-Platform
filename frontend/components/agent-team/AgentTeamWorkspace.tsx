...
export default function AgentTeamWorkspace({ role }: { role: Role }) {
  ...
  const loadData = useCallback(async (quiet = false) => {
    try {
      ...
      const [groupsResponse, runsResponse] = await Promise.all([api.get('/groups?limit=50'), api.get('/agent-team/runs?limit=20')]);
    ...
    <div className="relative min-h-full pb-12">
      ...
      <div className="relative grid gap-6 2xl:grid-cols-[380px_minmax(0,1fr)]">
        <aside className="space-y-5">
          <section className="rounded-[26px] border border-slate-200/80 bg-white/90 p-5 shadow-[0_20px_60px_rgba(15,23,42,.08)] backdrop-blur-xl">
            ...
            <div className="space-y-4">
              ...
              <button onClick={createRun} disabled={creating || !groupId} className="group flex w-full items-center justify-center gap-2 rounded-xl bg-cardinal px-4 py-3 text-sm font-bold text-white shadow-[0_12px_28px_rgba(210,35,42,.25)] transition hover:-translate-y-0.5 hover:bg-cardinal-dark disabled:cursor-not-allowed disabled:opacity-50">{creating ? <Loader2 className="h-4 w-4 animate-spin" /> : <Bot className="h-4 w-4" />}{creating ? 'Deploying agents…' : 'Start evidence review'}<ArrowRight className="h-4 w-4 transition group-hover:translate-x-1" /></button>
            </div>
          </section>
        ...
        <main className="min-w-0 space-y-6">
          {loading ? <div className="flex min-h-[560px] items-center justify-center rounded-[28px] border border-slate-200 bg-white"><Loader2 className="h-7 w-7 animate-spin text-cardinal" /></div> : !activeRun ? <div className="flex min-h-[560px] flex-col items-center justify-center rounded-[28px] border border-dashed border-slate-300 bg-white/70 px-8 text-center"><div className="mb-5 rounded-[22px] bg-slate-950 p-5 text-white"><BrainCircuit className="h-8 w-8" /></div><h2 className="font-display text-2xl text-slate-950">Your agent team is ready</h2><p className="mt-2 max-w-md text-sm leading-6 text-slate-500">Select a project, add its current context, and deploy a traceable review mission.</p></div> : <>
