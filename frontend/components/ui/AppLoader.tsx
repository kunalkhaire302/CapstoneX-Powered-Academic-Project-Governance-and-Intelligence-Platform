...
export default function AppLoader({ compact = false, label = 'Preparing your workspace' }: AppLoaderProps) {
  return (
    ...
    >
      ...
      <div className="absolute left-1/2 top-1/2 h-[28rem] w-[28rem] -translate-x-1/2 -translate-y-1/2 rounded-full bg-brand/10 blur-[100px]" aria-hidden="true" />
      ...
      >
        <div className="relative grid h-24 w-24 place-items-center" aria-hidden="true">
          ...
          <div className="loader-orbit-reverse absolute inset-[10px] rounded-full border border-brand/30" />
          ...
          <span className="absolute left-1/2 top-0 h-2 w-2 -translate-x-1/2 rounded-full bg-brand shadow-[0_0_18px_rgba(255,72,79,.9)]" />
        </div>

        <div className="mt-7 text-center">
          <p className="font-display text-2xl tracking-[-0.02em]">Capstone<span className="text-brand-400">X</span></p>
        ...
        <div className="mt-7 h-px w-40 overflow-hidden bg-white/10" aria-hidden="true">
          <div className="loader-progress h-full w-1/2 bg-gradient-to-r from-transparent via-brand to-transparent" />
        </div>
      </motion.div>
